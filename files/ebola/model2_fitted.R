...
deriv (S) <- - S / N * (beta_r * (I_r + C_r) + beta_d * (I_d + C_d))

deriv (E_1) <- S / N * (beta_r * (I_r + C_r) + beta_d * (I_d + C_d)) - 
  gamma_1 * E_1
...
I_init <- user(21.21, min = 0) # initial number of infected individuals ### default value was fitted to data
